...
vec2 grid2(in vec2 st,in float row,in float col){
  ...
  // 奇数行偏移正的,偶数偏移负的
  p.x+=step(0.,2.01-m)*step(1.,mod(p.y,2.))*m*1.;
  p.x-=step(0.,2.01-m)*(1.-step(1.,mod(p.y,2.)))*m*1.;
  
  p.y+=step(0.,m-2.01)*step(1.,mod(p.x,2.))*m*1.;
  p.y-=step(0.,m-2.01)*(1.-step(1.,mod(p.x,2.)))*m*1.;
